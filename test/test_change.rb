...
class TestChange < Minitest::Test
  def test_invalid_change
    assert_raises_with_message RuntimeError, /Invalid Change Action/ do
      change("~", 0, "element")
    end

    assert_raises_with_message RuntimeError, /Invalid Position Type/ do
      change("+", 3.5, "element")
    end
  end
  ...
  def test_from_a
    ...
    assert_raises_with_message RuntimeError, "Invalid change array format provided." do
      change_from_a(["+", 0])
    end
...
class TestContextChange < Minitest::Test
  ...
  def test_invalid_change
    ...
    assert_raises_with_message RuntimeError, "Invalid (Old) Position Type" do
      change("+", 3.5, "old", 0, "new")
    end

    assert_raises_with_message RuntimeError, "Invalid (New) Position Type" do
